...
export default function NavItem({ to, label, active}) {
  return (
    <li className="relative group">
      ...
      <div
        className={`
          ${active ? 'opacity-0' : 'absolute top-0 z-10 flex justify-center w-full opacity-0 scale-50 rotate-[-95deg] transition-all duration-200 ease-out group-hover:opacity-100 group-hover:scale-100 group-hover:-translate-y-[35px]'}
        `}
      >
        <img
          src={swirl}
          alt="swirl"
          className={`
            ${active ? 'opacity-0' : 'transition-transform duration-300 ease-out delay-100 group-hover:rotate-[95deg]'}
          `}
        />
      </div>
      <img 
